...
const tvBrand = inventory.map((tv => tv.brand));
console.log(tvBrand);

element = document.getElementById("brand");
element.textContent = tvBrand;
...
function TvObjectList(tvArray) {

    const tvObjecten = tvArray.map((tv) => ` ${tv.brand}`);
    const objectList = document.getElementById('objectList');
    objectList.textContent = tvObjecten;
}

TvObjectList(inventory);
